...
BillboardGeometry::BillboardGeometry( Qt3DCore::QNode *parent )
  : Qt3DExtras::QPlaneGeometry( parent )
  ...
  , mInstanceBuffer( new Qt3DRender::QBuffer( Qt3DRender::QBuffer::VertexBuffer, this ) )
{
  ...
  mPositionAttribute->setVertexSize( 4 );
  mPositionAttribute->setName( Qt3DRender::QAttribute::defaultPositionAttributeName() );
  mPositionAttribute->setName( QStringLiteral( "pos_id" ) );
  mPositionAttribute->setDivisor( 1 );
  mPositionAttribute->setByteStride( 4 * sizeof( float ) );
...
QMatrix4x4 BillboardGeometry::normalMatrix(QMatrix4x4 mat)
{
...
void BillboardGeometry::setPoints(const QVector<QVector3D> &vertices)
{
    ...
    int idx = 0;
    // Need to start at 1 because background is black, i.e. "ID" 0
    uint id_index = 1;
